...
/*
 *  Name of the test  : PROPOSED_INVOICE_API_with_PROPTOT_remove_system_application
...
public class PROPOSED_INVOICE_API_with_PROPOSED_remove_system_application extends commonBase {
	...
	public void f() throws Exception {
		...
		TCID = "IES_652";																						//Test case ID
		parent = repo.setParent(TCID + " Proposed_invoce");
		test = repo.setChildNode("PROPOSED_INVOICE_API_with_PROPOSED_remove_system_application", parent);		//Test case name
		resultset = data.ConnectAndQueryEPPIXServer("", "", "","select first 2 vam_msisdn_no,ebb_unbilled,* from vam_active_msisdn inner join ebb_epx_billsofar_bulk ON vam_msisdn_no = ebb_msisdn_no where vam_stat_code IN(1,4) AND vam_intern_tariff not in('PRE','C3D') and ebb_unbilled > 100");
		...
		//Execute stored proc to obtain totals
		String propInvoice = "EXECUTE PROCEDURE 'informix'.epx_proposed_api('PROPOSED','"+ vam_msisdn_no[1] +"','','eppix','Molokommes',user)";
		...
		try {
			...
			repo.childLogPass("Obtain Execute_PROPOSED SP Details, No system appliction found.", "No System Parameter was specified.", "No System Parameter was specified.", false, parent, utils);
		}
