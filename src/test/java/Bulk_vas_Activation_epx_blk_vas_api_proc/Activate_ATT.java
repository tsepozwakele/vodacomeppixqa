...
public class Activate_ATT extends commonBase {
...
public void f() throws Exception {
		  TCID = "IES-380";                                         //JIRA ticket number 
		  	parent = repo.setParent(TCID +"Activate_ATT_N2N = N");
		  		test = repo.setChildNode("Activate_ATT_N2N = N", parent);
		...
		String strActivateATT = "EXECUTE PROCEDURE epx_blk_vas_api_proc('["+ sAccountNumber[1] +" ]','','["+ sSalesmanCode[1] +"]','ATT','N','["+ sEmail[1] +"]','["+ sAction+"]','["+sUser+"]')";
				resultset1 = data.ConnectAndQueryEPPIXServer("", "", "", strActivateATT);	
				...
				if (sResults2 != null) {
					...
					if (sStatus[1].equals("C") && sMessage[1].contains("Request has been succefully logged: Ref: ")) {
						// System.out.println(Compare[1]+","+Compare[1]);
						 	repo.childLogPass("Activate_ATT", "Status="+sStatus[1]+", Message="+sMessage[1]+"","Status="+sStatus[1]+", Message="+sMessage[1]+"", false, test, utils); 
						 	
					}

					else {
						System.out.println("Did not Activate_ATT service, Please Investigate");
						repo.childLogFail("Verify ATT Activated successfully",
								"ATT Activated successfully", sResults2, false, test, utils);
					}
				}
