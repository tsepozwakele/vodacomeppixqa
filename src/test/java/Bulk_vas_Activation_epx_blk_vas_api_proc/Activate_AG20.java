...
public class Activate_AG20 extends commonBase {
...
public void f() throws Exception {
		  TCID = "IES-378";                                         //JIRA ticket number 
		  	parent = repo.setParent(TCID +"Activate_AG20(Once off bundle)N2N = N");
		  		test = repo.setChildNode("Activate_AG20(Once off bundle)N2N = N", parent);
		...
		String strActivateAG20 = "EXECUTE PROCEDURE epx_blk_vas_api_proc('["+ sAccountNumber[1] +" ]','','["+ sSalesmanCode[1] +"]','AG20','N','["+ sEmail[1] +"]','["+ sAction+"]','["+sUser+"]')";
				resultset1 = data.ConnectAndQueryEPPIXServer("", "", "", strActivateAG20);	
				...
				if (sResults2 != null) {
					...
					if (sStatus[1].equals("C") && sMessage[1].contains("Request has been succefully logged: Ref: ")) {
						// System.out.println(Compare[1]+","+Compare[1]);
						 	repo.childLogPass("Activate_AG20", "Status="+sStatus[1]+", Message="+sMessage[1]+"","Status="+sStatus[1]+", Message="+sMessage[1]+"", false, test, utils); 
						 	
					}

					else {
						System.out.println("Did not Activate_AG20");
						repo.childLogFail("Verify AG20 Activated successfully",
								"AG20 Activated successfully", sResults2, false, test, utils);
					}
				}
