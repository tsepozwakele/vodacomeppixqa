...
public class Activate_B26 extends commonBase {
...
public void f() throws Exception {
		  TCID = "IES-382";                                         //JIRA ticket number 
		  	parent = repo.setParent(TCID +"Activate_B26_N2N = N");
		  		test = repo.setChildNode("Activate_B26_N2N = N", parent);
		...
		String strActivateB26 = "EXECUTE PROCEDURE epx_blk_vas_api_proc('["+ sAccountNumber[1] +" ]','','["+ sSalesmanCode[1] +"]','ATT','N','["+ sEmail[1] +"]','["+ sAction+"]','["+sUser+"]')";
				resultset1 = data.ConnectAndQueryEPPIXServer("", "", "", strActivateB26);	
				...
				if (sResults2 != null) {
					...
					if (sStatus[1].equals("C") && sMessage[1].contains("Request has been succefully logged: Ref: ")) {
						// System.out.println(Compare[1]+","+Compare[1]);
						 	repo.childLogPass("Activate_B26", "Status="+sStatus[1]+", Message="+sMessage[1]+"","Status="+sStatus[1]+", Message="+sMessage[1]+"", false, test, utils); 
						 	
					}

					else {
						System.out.println("Did not Activate_B26 service, Please Investigate");
						repo.childLogFail("Verify B26 Activated successfully",
								"B26 Activated successfully", sResults2, false, test, utils);
					}
				}
