...
public class Activate_CLIP extends commonBase {
...
public void f() throws Exception {
		  TCID = "IES-386";                                         //JIRA ticket number 
		  	parent = repo.setParent(TCID +"Activate_CLIP N2N = N");
		  		test = repo.setChildNode("Activate_CLIP N2N = N", parent);
		...
		String strActivateCLIP = "EXECUTE PROCEDURE epx_blk_vas_api_proc('["+ sAccountNumber[1] +" ]','','["+ sSalesmanCode[1] +"]','ATT','N','["+ sEmail[1] +"]','["+ sAction+"]','["+sUser+"]')";
				resultset1 = data.ConnectAndQueryEPPIXServer("", "", "", strActivateCLIP);	
				...
				if (sResults2 != null) {
					...
					if (sStatus[1].equals("C") && sMessage[1].contains("Request has been succefully logged: Ref: ")) {
						...
						System.out.println("Did not Activate_CLIP service, Please Investigate");
						repo.childLogFail("Verify Activate_CLIP Activated successfully",
								"Activate_CLIP Activated successfully", sResults2, false, test, utils);
					}
				}
