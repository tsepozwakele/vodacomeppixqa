...
public class Swap_a_Subscriber_without_change_address extends commonBase {
	...
	public void f() throws Exception {
		...
		test = repo.setChildNode("Swap_a_Subscriber_without_change_address", parent);				//Test case name
		
		//Obtain account_no and sbd_dialling_no with any service e.g. IBIL,C3DL 
		...
		if (Obtained_old_acc != null) {
			...
			System.out.println("Obtain account_no and sbd_dialling_no with C3DL service:old_account_no");
			repo.ExtentLogFail("", false, utils, test);
		}
		//Obtain current email address and sbd_account_no without C3DL/IBIL service  
		...
		if (sResults != null) 
			{
				...
				System.out.println("Obtain account_no without C3DL service,new_account_no: ["+ account_no[1] + "], and referral_no: [" + referral_no[1]+ "] obtained");
				repo.ExtentLogPass("Obtain account_no without C3DL service,new_account_no:["+ account_no[1] + "],and referral_no: [" + referral_no[1] + "],]",false, utils, test);
				
			}
		
		else {
			System.out.println("Obtain account_no without IBIL service,new_account_no was not successfull");
			repo.ExtentLogFail("Obtain account_no without IBIL service,new_account_no was not successfull", false, utils, test);
		}
		
		 //Execute SUBCUS UPDATE stored procedure to in attempt to swap old with new account no
		String swapSubscriber = "execute procedure ss_epx_api_proc('SUBCUS UPDATE','"+vam_msisdn_no[1] +"','"+ account_no[1] +"','','DEFCHRG=[N]','','','','','EPPIX','MAINTENANCE',user,user,'REASON=[TNFR],DEALER=[VODAC],OC3RSN=[AA],SMS=[N],OC3_IGN=[Y],')";
		...
		try {
			...
			repo.childLogPass("Verify SUBCUS UPDATE, without change address was successful,No Change Address was specified.", "No Change Address Parameter was specified.", "No Change Address Parameter was specified.", false, test, utils);
		}
