...
public class Swap_a_Subscriber_without_system_name extends commonBase {
	...
	public void f() throws Exception {
		...
		TCID = "IES_798";																	//Test Case ID
		parent = repo.setParent(TCID + " SUBCUSSWAP.4ge");
		test = repo.setChildNode("Swap_a_Subscriber_without_system_name", parent);				//Test case name
		...
		String swapSubscriber = "execute procedure ss_epx_api_proc('SUBCUS UPDATE','"+ sbd_bill_ac_no[1] +"','"+account_no[1] +"','Y','DEFCHRG=[N]','','','','','','MAINTENANCE',user,user,'REASON=[TNFR],DEALER=[VODAC],OC3RSN=[AA],SMS=[N],OC3_IGN=[Y],')";
		...
		try {
			...
			repo.childLogPass("Verify SUBCUS UPDATE, without system name  was successful,No MSISDN number was specified.", "No System Parameter was specified.", "No System Parameter was specified.", false, test, utils);
		}
