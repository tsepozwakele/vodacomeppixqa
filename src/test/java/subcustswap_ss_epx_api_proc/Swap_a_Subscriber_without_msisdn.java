...
public class Swap_a_Subscriber_without_msisdn extends commonBase {
	...
	public void f() throws Exception {
		...
		test = repo.setChildNode("Swap_a_Subscriber_without_msisdn", parent);				//Test case name
		...
		if (sResults != null) 
		 ...
		 //Execute SUBCUS UPDATE stored procedure in attempt to swap old and new account number
		String swapSubscriber = "execute procedure ss_epx_api_proc('SUBCUS UPDATE','','"+account_no[1] +"','Y','DEFCHRG=[N]','','','','','EPPIX','MAINTENANCE',user,user,'REASON=[TNFR],DEALER=[VODAC],OC3RSN=[AA],SMS=[N],OC3_IGN=[Y],')";
		...
		try {
			...
			repo.childLogPass("Verify SUBCUS UPDATE, without msisdn  was successful,No MSISDN number was specified.", "No MSISDN number was specified.", "No MSISDN number was specified.", false, test, utils);
		}
