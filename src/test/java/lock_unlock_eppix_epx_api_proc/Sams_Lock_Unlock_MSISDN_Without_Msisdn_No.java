...
public class Sams_Lock_Unlock_MSISDN_Without_Msisdn_No extends commonBase {
	...
	public void LockUnlock() throws Exception {
		...
		TCID = "???";
		parent = repo.setParent(TCID + " lock_unlock_eppix_epx_api_pro");					//Test Case No
		test = repo.setChildNode("Sams_Lock_Unlock_MSISDN_Without_MSISND_NO", parent);		//Test case name
		...
		String strLockUnlock = "EXECUTE PROCEDURE ext_services:sams_lock_unlock('L','','SAMS','Y','molokommes','SAMS');";
		...
		if (sResults2 != null) {
			...
			for (int i = 1; i < 3; i++) {
				...
				if (vstatus[1].equals("E") && vmessage[1].equals("pMsisdn must be specified for sams_lock_unlock!")) {

					System.out.println(
							"Verify sams lock unlock without send Msisdn was not successful, sams lock unlock without send Msisdn was NOT successful as expected. vstatus: ["
