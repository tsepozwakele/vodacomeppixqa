...
public class Sams_Lock_Unlock_MSISDN_Without_username_not_EPPIX extends commonBase {
	...
	public void f() throws Exception {
		...
		TCID = "IES_783";																		//Test Case ID ( 787,792) duplicate
		parent = repo.setParent(TCID + " lock_unlock_eppix_epx_api_pro");
		test = repo.setChildNode("Sams_Lock_Unlock_MSISDN_Without_username_not_EPPIX", parent);	 //Test case name
		...
		String strLockUnlock = "EXECUTE PROCEDURE ext_services:sams_lock_unlock('L','"+msisdn_no[1]+"','SAMS','Y','','SAMS');";
		...
		//Verify the lock was not successful
				if (vstatus[1].equals("E") && vmessage[1].equals("pUser must be specified for sams_lock_unlock!")) {

					System.out.println(
							"Verify sams lock unlock without send User was not successful, sams lock unlock without send User was NOT successful as expected. vstatus: ["
					...
					System.out.println(
							"Verify sams lock unlock without send User was successful, sams lock unlock without send User was successful. vstatus: ["
		...
		resultset = data.ConnectAndQueryEPPIXServer("", "", "","select vam_msisdn_no,vam_subscriber_id,vam_sim_no,vam_stat_code from vam_active_msisdn where vam_msisdn_no ='663286324000'");
