...
public class Sams_Lock_Unlock_MSISDN_Without_Lock_Type extends commonBase {
	...
	public void lockUnlock() throws Exception {
		...
		TCID = "IES_784";
		parent = repo.setParent(TCID + " lock_unlock_eppix_epx_api_pro");  //Test case ID
		test = repo.setChildNode("Sams_Lock_Unlock_MSISDN_Without_lock_type", parent);  // Test case name
		...
		String strLockUnlock = "EXECUTE PROCEDURE ext_services:sams_lock_unlock('','"+msisdn_no[1]+"','SAMS','Y','molokommes','SAMS');";
		...
		if (sResults2 != null) {
			...
			for (int i = 1; i < 3; i++) {
				...
				if (vstatus[1].equals("E") && vmessage[1].equals("pLockType must be specified for sams_lock_unlock!")) {

					System.out.println(
							"Verify sams lock unlock without send LockType was not successful, sams lock unlock without send LockType was NOT successful as expected. vstatus: ["
					...
					System.out.println(
							"Verify sams lock unlock without send LockType was successful, sams lock unlock without send User was successful. vstatus: ["
