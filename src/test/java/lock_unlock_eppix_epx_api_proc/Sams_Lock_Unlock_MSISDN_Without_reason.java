/*
 *  Name of the test  : Sams_Lock_Unlock_MSISDN_Without_username_not_EPPIX
...
public class Sams_Lock_Unlock_MSISDN_Without_reason extends commonBase {
	...
	@Test
	public void Lock_Unlock() throws Exception {

		TCID = "???";																		//Test Case ID ( 787,792) duplicate
		parent = repo.setParent(TCID + " lock_unlock_eppix_epx_api_pro");
		test = repo.setChildNode("Sams_Lock_Unlock_MSISDN_Without_username_Reason", parent);	 //Test case name
		
		//Obtain active 12 digits msisdn_no from vam_active_msisdn
		...
		if (resultset != null) {
			...
			System.out.println("Obtain active 12 digits msisdn_no from vam_active_msisdn,vam_msisdn_no: ["
					+ msisdn_no[1] + "], vam_subscriber_id: [" + subscriber_id[1] + "], vam_sim_no: [" + sim_no[1]
					+ "] and vam_contr_terminat : [" + contr_terminat[1] + "] obtained");
			repo.childLogPass("Obtain msisdn_no vam_active_msisdn,vam_msisdn_no", msisdn_no[1], msisdn_no[1], false,
					test, utils);
			repo.childLogPass("Obtain subscriber_id vam_active_msisdn,vam_msisdn_no", subscriber_id[1],
					subscriber_id[1], false, test, utils);
			repo.childLogPass("Obtain subscriber_id vam_active_msisdn,vam_msisdn_no", sim_no[1], sim_no[1], false, test,
					utils);
			repo.childLogPass("Obtain subscriber_id vam_active_msisdn,vam_msisdn_no", contr_terminat[1],
					contr_terminat[1], false, test, utils);
		...
		//EXECUTE PROCEDURE sams_lock_unlock to lock msisdn

		String strLockUnlock = "EXECUTE PROCEDURE ext_services:sams_lock_unlock('L','"+msisdn_no[1]+"','','Y','Molokommes','SAMS');";
		resultset = data.ConnectAndQueryEPPIXServer("", "", "", strLockUnlock);
		...
		if (sResults2 != null) {
			...
			for (int i = 1; i < 3; i++) {
				msisdn_no[i] = data.getCellData("vmsisdn", i, null, null, resultset, "SQLSERVER").trim().toString();
				vstatus[i] = data.getCellData("vstatus", i, null, null, resultset, "SQLSERVER").trim().toString();
				vmessage[i] = data.getCellData("vmessage", i, null, null, resultset, "SQLSERVER").trim().toString();
				
		//Verify the lock was not successful
				if (vstatus[1].equals("E") && vmessage[1].equals("pReason must be specified for sams_lock_unlock!")) {

					System.out.println(
							"Verify sams lock unlock without send Reason was not successful, sams lock unlock without send Reason was NOT successful as expected. vstatus: ["
					...
					System.out.println(
							"Verify sams lock unlock without send Reason was successful, sams lock unlock without send Reason was successful. vstatus: ["
									+ vstatus[1] + "],and vmessage:[" + vmessage[1] + "].Please Investigate");
					repo.ExtentLogFail(
							"Verify sams lock unlock without send Reason was successful, sams lock unlock without send User was successful. vstatus: ["
									+ vstatus[1] + "],and vmessage:[" + vmessage[1] + "].Please Investigate",
							false, utils, test);
		...
		//Obtain record from vam_active_msisdn and verify vam_stat_code

		resultset = data.ConnectAndQueryEPPIXServer("", "", "","select vam_msisdn_no,vam_subscriber_id,vam_sim_no,vam_stat_code from vam_active_msisdn where vam_msisdn_no ='"+ msisdn_no[1] +"'");
