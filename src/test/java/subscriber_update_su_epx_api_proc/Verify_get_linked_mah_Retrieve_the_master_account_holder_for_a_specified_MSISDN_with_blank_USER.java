...
public class Verify_get_linked_mah_Retrieve_the_master_account_holder_for_a_specified_MSISDN_with_blank_USER extends commonBase {
  ...
  public void f() throws Exception {
	  TCID = "IES-808";                            //Jira ticket 
	  	parent = repo.setParent(TCID +"Verify_get_linked_mah_Retrieve_the_master_account_holder_for_a_specified_MSISDN_with_blank_USER");
	  	test = repo.setChildNode("Verify_get_linked_mah_Retrieve_the_master_account_holder_for_a_specified_MSISDN_with_blank_USER", parent);
		...
		String strSwapMAH = "execute procedure su_epx_api('GET_LINKED_ACCS','MSISDN=[" +sMSISDN[1]
				+ "],USER=[],','','','','')";
		...
		// Get the query results and store them in a variable
				...
				if (sResults2 != null) {
					...
					if (Compare[1].equals("EPX_STATUS=[E]") && Compare[2].equalsIgnoreCase("MESSAGE=[Linked accounts successfully retrieved]")) {
						 System.out.println(Compare[1]+","+Compare[2]);
						 	repo.childLogPass("Verify_get_linked_mah_Retrieve_the_master_account_holder_for_a_specified_MSISDN_with_blank_USER",
				                    ""+Compare[1]+","+Compare[2]+"",
				                     ""+Compare[1]+","+Compare[2]+"", false, test, utils); 
					}

					else {
						System.out.println("Did not update subscribers_linked_to_a_specified_MSISDN_with_blank_USER");
