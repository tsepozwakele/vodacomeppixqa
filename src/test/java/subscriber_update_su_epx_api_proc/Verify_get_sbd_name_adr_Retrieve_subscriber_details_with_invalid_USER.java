...
public class Verify_get_sbd_name_adr_Retrieve_subscriber_details_with_invalid_USER extends commonBase {
  ...
  public void f() throws Exception {
	  TCID = "691";
	  	parent = repo.setParent(TCID +"Verify_get_sbd_name_adr_Retrieve_subscriber_details_with_invalid_USER");
	  	test = repo.setChildNode("Verify_get_sbd_name_adr_Retrieve_subscriber_details_with_invalid_USER", parent);
		...
		String strSwapMAH = "execute procedure su_epx_api('GET_SBD_NAME_ADR','MSISDN=["+ sMSISDN[1]
				+ "],USER=[!!!!],','','','','')";
		...
		// Get the query results and store them in a variable
				...
				if (sResults2 != null) {
					...
					if (Compare[1].equals("EPX_STATUS=[C]") && Compare[2].equalsIgnoreCase("MESSAGE=[Data for subscriber ]")) {
						 System.out.println(Compare[1]+","+Compare[2]);
						 	repo.childLogPass("Verify upd_sbd_basic- Update subscriber details with invalid length User",
				                    ""+Compare[1]+","+Compare[2]+"",
				                     ""+Compare[1]+","+Compare[2]+"", false, test, utils); 
					}

					else {
						System.out.println("Did not update subscriber details with invalid User");
