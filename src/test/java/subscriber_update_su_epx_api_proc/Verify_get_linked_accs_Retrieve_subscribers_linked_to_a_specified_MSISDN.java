...
public class Verify_get_linked_accs_Retrieve_subscribers_linked_to_a_specified_MSISDN extends commonBase {
  ...
  public void f() throws Exception {
	  TCID = "IES-810";                            //Jira ticket 
	  	parent = repo.setParent(TCID +"Verify_get_linked_accs_Retrieve_subscribers_linked_to_a_specified_MSISDN");
	  	test = repo.setChildNode("Verify_get_linked_accs_Retrieve_subscribers_linked_to_a_specified_MSISDN", parent);
		...
		// Get the query results and store them in a variable
				...
				if (sResults2 != null) {
					...
					if (Compare[1].equals("EPX_STATUS=[C]") && Compare[2].equalsIgnoreCase("MESSAGE=[Linked accounts successfully retrieved]")) {
						 System.out.println(Compare[1]+","+Compare[2]);
						 	repo.childLogPass("Verify_get_linked_accs_Retrieve_subscribers_linked_to_a_specified_MSISDN",
				                    ""+Compare[1]+","+Compare[2]+"",
				                     ""+Compare[1]+","+Compare[2]+"", false, test, utils); 
					}

					else {
						System.out.println("Did not update subscribers_linked_to_a_specified_MSISDN");
