...
public class Verify_query_mah_flag_Retrieve_master_account_holder_status_invalid_MSISDN  extends commonBase {
  ...
  public void f() throws Exception {
	  TCID = "IES-805";                            //Jira ticket 
	  	parent = repo.setParent(TCID +"Verify_query_mah_flag_Retrieve_master_account_holder_status_invalid_MSISDN");
	  	test = repo.setChildNode("Verify_query_mah_flag_Retrieve_master_account_holder_status_invalid_MSISDN", parent);
		...
		String strSwapMAH = "execute procedure que_epx_api('QUERY_MAH_FLAG','824133603!!0','','','','')";
		...
		// Get the query results and store them in a variable
				...
				if (sResults2 != null) {
					...
					if (Compare[1].equals("EPX_STATUS=[E]") && Compare[2].equalsIgnoreCase("MESSAGE=[Subscriber not found]")) {
						 System.out.println(Compare[1]+","+Compare[2]);
						 	repo.childLogPass("Verify_query_mah_flag_Retrieve_master_account_holder_status_invalid_MSISDN",
				                    ""+Compare[1]+","+Compare[2]+"",
				                     ""+Compare[1]+","+Compare[2]+"", false, test, utils); 
					}

					else {
						System.out.println("Did not update master account holder status invalid MSISDN");
