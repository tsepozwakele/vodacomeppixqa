...
public class Verify_query_mah_flag_Retrieve_master_account_holder_status_without_MSISDN extends commonBase {
  ...
  public void f() throws Exception {
	  TCID = "803";
	  	parent = repo.setParent(TCID +"Verify_query_mah_flag_Retrieve_master_account_holder_status_without_MSISDN");
	  	test = repo.setChildNode("Verify_query_mah_flag_Retrieve_master_account_holder_status_without_MSISDN", parent);
		...
		String strSwapMAH = "execute procedure que_epx_api('QUERY_MAH_FLAG','','','','','')";
		...
		// Get the query results and store them in a variable
				...
				if (sResults2 != null) {
					...
					if (Compare[1].equals("EPX_STATUS=[E]") && Compare[2].equalsIgnoreCase("MESSAGE=[Attribute list empty]")) {
						 System.out.println(Compare[1]+","+Compare[2]);
						 	repo.childLogPass("Verify_query_mah_flag_Retrieve_master_account_holder_status_without_MSISDN",
				                    ""+Compare[1]+","+Compare[2]+"",
				                     ""+Compare[1]+","+Compare[2]+"", false, test, utils); 
					}

					else {
						System.out.println("Did not update master account holder status without MSISDN");
