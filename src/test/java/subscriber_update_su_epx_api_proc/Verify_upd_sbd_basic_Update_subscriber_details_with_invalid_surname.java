...
public class Verify_upd_sbd_basic_Update_subscriber_details_with_invalid_surname  extends commonBase {
  ...
  public void f() throws Exception {
	  TCID = "706";
	  	parent = repo.setParent(TCID +"Verify upd_sbd_basic Update subscriber details with invalid Surname");
	  	test = repo.setChildNode("Verify upd_sbd_basic Update subscriber details with invalid Surname", parent);
		...
		String strSwapMAH = "execute procedure su_epx_api('UPD_SBD_BASIC','MSISDN=[" + sMSISDN[1]
				+ "],USER=[duraanwy],OC3PSWD=[gn225],LEGALPSWD=[ga],SSPSWD=[1ss],TEST_SPONS_SIM=[2],COST_CENTRE=[181700]','sbd_address1=[15 Piet str], sbd_address2=[ ], sbd_address3=[WingatePark], sbd_address4=[PRETORIA],sbd_postcode=[0002],sbd_sub_tel=[0119734436],sbd_con_tel=[0121234567], sbd_email=[some@vodac.co.za],sbd_title=[MR], sbd_firstname=[Stainless],sbd_surname=[" + sSurname[1]
				...
				if (sResults2 != null) {
					...
					if (Compare[1].equals("EPX_STATUS=[C]") && Compare[2].equalsIgnoreCase("MESSAGE=[Update successful]")) {
						 System.out.println(Compare[1]+","+Compare[2]);
						 	repo.childLogPass("Verify upd_sbd_basic- Update subscriber details with invalid Surname",
				                    ""+Compare[1]+","+Compare[2]+"",
				                     ""+Compare[1]+","+Compare[2]+"", false, test, utils); 
					}

					else {
						System.out.println("Did not update subscriber details with invalid Surname");
