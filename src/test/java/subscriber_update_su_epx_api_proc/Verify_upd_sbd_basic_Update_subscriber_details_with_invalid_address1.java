...
public class Verify_upd_sbd_basic_Update_subscriber_details_with_invalid_address1 extends commonBase {
  ...
  public void f() throws Exception {
	  TCID = "709";
	  	parent = repo.setParent(TCID +"Verify_upd_sbd_basic_Update_subscriber_details_with_invalid_address1");
	  	test = repo.setChildNode("Verify_upd_sbd_basic_Update_subscriber_details_with_invalid_address1", parent);
	  	...
	  	if (resultset != null) {
			...
			System.out.println("Subscriber address1 : " + sAddress1[1]);
			repo.childLogPass("Getting MSISDN and Subscriber Address1","MSISDN obtained :" + sMSISDN[1] + " and Subscriber Address1: " + sAddress1[1],
					"MSISDN obtained :" + sMSISDN[1] + " and Subscriber Address1 :" + sAddress1[1], false, test, utils);

		} else {
			System.out.println("NO records returned");
			repo.childLogFail("Getting MSISDN and Address1",
					"MSISDN obtained :" + sMSISDN[1] + " and Subscriber Address1: " + sAddress1,
					"MSISDN obtained :" + sMSISDN[1] + " and Subscriber Address1 :" + sAddress1, false, test, utils);
		
		}

		String strSwapMAH = "execute procedure su_epx_api('UPD_SBD_BASIC','MSISDN=[" + sMSISDN[1]
				+ "],USER=[duraanwy],OC3PSWD=[gn225],LEGALPSWD=[ga],SSPSWD=[1ss],TEST_SPONS_SIM=[2],COST_CENTRE=[181700]','sbd_address1=[!!!], sbd_address2=[ ], sbd_address3=[WingatePark], sbd_address4=[PRETORIA],sbd_postcode=[0002],sbd_sub_tel=[0119734436],sbd_con_tel=[0121234567], sbd_email=[some@vodac.co.za],sbd_title=[MR], sbd_firstname=[Wynand],sbd_surname=[du Raan], sbd_gender=[M], sbd_dob=[24061978], sbd_fax=[0116535414],','sba_analysis_1=[FAX],sba_analysis_3=[N],sba_analysis_4=[031],sba_analysis_5=[],sba_analysis_6=[B],sba_cust_dept=[cust dept],sba_comments=[make another comment],sba_id_number=[6911245117082],','sba3_email=[wynand@vodacom.co.za],sba3_email_flag=[Y],','sbv_credit_limit=[500],sbv_percent_check=[80],')";
				...
				if (sResults2 != null) {
					...
					if (Compare[1].equals("EPX_STATUS=[C]") && Compare[2].equalsIgnoreCase("MESSAGE=[Update successful!]")) {
						 System.out.println(Compare[1]+","+Compare[2]);
						 	repo.childLogPass("Verify upd_sbd_basic- Update subscriber details with invalid Address1",
				                    ""+Compare[1]+","+Compare[2]+"",
				                     ""+Compare[1]+","+Compare[2]+"", false, test, utils); 
					}

					else {
						System.out.println("Did not update subscriber details with invalid Address1");
